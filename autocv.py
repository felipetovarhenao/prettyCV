...
class CV:
    ...
    def __new_subsection(self, name):
        self.__insert_break()
    ...
    def __insert_break(self, n_units=1):
        gap = Pt(7 * n_units)
    ...
    def __make_entry_table(self, parent: object, items: list, handler: Callable, date_getter: Callable):
        ...
        for i, item in enumerate(items):
            ...
            if date != last_date:
                left.paragraphs[0].add_run(date)
            last_date = date
            handler(cell=right, item=item, id=i)
    ...
    def parse_work_experience(self):
        def handler(cell=None, id=None, item=None):
            position = item
            p = cell.paragraphs[0]
        ...
        academic_work, other_work = work.pop('academic', None), work.pop('other positions')
